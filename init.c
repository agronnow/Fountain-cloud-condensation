 ...
 *********************************************************************** */
{
  ...
  #endif
  nWind = g_tabulardensity[g_curidx];
  #endif
  #ifdef VAR_MU
  if (firstcall)
  {
  ...
  #ifdef VAR_MU
  double T = PrsCloud*KELVIN/Numdens;
 ...
 *********************************************************************** */
{
  ...
  double nCloud = g_inputParam[RHO_C];
  double TCloud = g_inputParam[TEMP_C];
  double muWind = g_inputParam[MU_C];
//  static double sqrt3 = sqrt(3.0);
  #ifdef VAR_MU
  muWind = GetMuFromTable(2.e6); //Assuming that T_wind >> 1e4 K
  #endif
  double RhoWind = g_inputParam[RHO_W]*muWind;
  #ifdef VARIABLE_INFLOW
  nWind = g_tabulardensity[g_curidx];
  #endif
#ifdef PRS_FROM_TEMP
  double PrsWind = nCloud*TCloud/KELVIN;
#else
  ...
  else if (side == X1_BEG)
  {
    if (box->vpos == CENTER)
    {
      BOX_LOOP(box,k,j,i)
      {
	for (nv = 0; nv < NVAR; nv++) {d->Vc[nv][k][j][i] = d->Vc[nv][KEND][JEND][IEND];}
	...
	if (InitRhoWind == 0) {InitRhoWind = g_tabulardensity[g_curidx]*muWind;}
        ...
        niter++;
	d->Vc[RHO][k][j][i] = g_tabulardensity[g_curidx]*muWind;
	#endif
	double B0 = 0;
        #if PHYSICS == MHD
	#ifdef VARIABLE_INFLOW
	...
	#else
        #ifdef BINIT_MICROGAUSS
        B0 = g_inputParam[BETA_W]*UNIT_MICROGAUSS;
        #else
...
//        d->Vc[PRS][k][j][i] = d->Vc[RHO][k][j][i]*PrsWind/InitRhoWind;
        #endif
	#if PHYSICS == MHD
...
//	print("B0: %f\n",B0);
    ...
    else if (box->vpos == X2FACE)
    {
      ...
      {
	#ifdef VARIABLE_INFLOW
	d->Vs[BX2s][k][j][i] = g_tabularbfield[g_curidx] * UNIT_MICROGAUSS;
	#else
	d->Vs[BX2s][k][j][i] = B0;//d->Vs[BX2s][k][j][IBEG];
	#endif
      }
      #endif
    }
    else if (box->vpos == X3FACE)
    {
      ...
      {
	#ifdef VARIABLE_INFLOW
	d->Vs[BX3s][k][j][i] = 0;//g_tabularbfield[g_curidx]/sqrt3 * UNIT_MICROGAUSS;
	#else
	d->Vs[BX3s][k][j][i] = 0;//d->Vs[BX3s][k][j][IBEG];
	#endif
      }
      #endif
    }
